...
public class deck {
   ...
   //every card put in play-hands, burners, the board, etc to keep track so the cards can be put pack when done
   private ArrayList<card> cardsinuse = new ArrayList<card>();
   //stricly whats on the board, so can be compared with hands to find out the best hand
...
/**
    public static boolean equal(deck testDeck)
    {   
        boolean retval = true;
        ...
        else
        {
            ...
            {
                if(deckofcards.get(i).isequal(testDeck.deckofcards.get(i)) == false)
                {
                    retval = false;
                    break;
                }
